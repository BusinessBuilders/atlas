...
Synthesising the vendor documentation surveyed, a complete SMB AI-receptionist dashboard has nine areas. Anything missing from the first six reads as unfinished to a buyer.

**1. Call log — the home screen.** Date/time, caller ID, duration, outcome, with **audio playback, full transcript and an AI-generated summary on every call**. This is universal in the packaged tier: Rosie ships "Call summaries, transcripts & recordings" on *every* plan ([heyrosie.com](https://heyrosie.com)); Dialzara includes "Call recordings" and "Call summaries of every conversation" on all plans ([dialzara.com/pricing](https://dialzara.com/pricing)); My AI Front Desk shows "Call recordings and transcripts… in the dashboard for review" ([AINORA review](https://ainora.lt/blog/my-ai-front-desk-review-alternatives-2026)); RingCentral records and transcribes every call automatically ([RingCentral](https://www.ringcentral.com/ai-receptionist.html)). A customer quote captures why it matters: *"it captures every call and delivers both a full dialog transcript AND the actual recording straight to us. No more wondering what was said"* ([heyrosie.com](https://heyrosie.com)).

**2. Message / lead inbox.** The structured output of calls — name, number, reason for calling — separate from the raw call list. The leading pattern is a **unified inbox spanning calls and texts**: Rosie's is explicitly "All Your Conversations, One Inbox… Every call and text conversation is logged right in your Rosie mobile app, alongside AI summaries and full transcripts" ([Rosie mobile app page](https://heyrosie.com/features/mobile-app)).

**3. Greeting, persona and instructions editor.** The owner sets business name, agent name, voice and greeting. Rosie ships "10+ agent voices", "Custom agent name & greeting" and "Custom FAQ & answers" on every plan ([heyrosie.com](https://heyrosie.com)); Dialzara offers "Over 50 premium voices" on all plans and a **self-editable prompt** even on the $29 tier ([dialzara.com/pricing](https://dialzara.com/pricing)). The interaction model varies: free-text prompt (Dialzara), guided form plus FAQ pairs (Rosie), **logic flows** (Goodcall — 1/3/25 flows by tier, [goodcall.com/pricing](https://www.goodcall.com/pricing)), or **intents** (Webex, where an admin adds an intent name plus a natural-language description such as *"Transfer callers who ask about physical therapy, rehabilitation, or injury recovery"* and a transfer destination — [Webex Help](https://help.webex.com/en-us/article/4chov0/AI-Receptionist-in-Webex-Calling)). Dialzara has gone a step further with an **AI Configurator**: "An AI assistant that builds and refines your agent. Describe what you need — it handles setup" ([dialzara.com/features](https://dialzara.com/features)).

**4. Knowledge base / FAQ.** Website-scrape ingestion is the standard entry point (see §2.3), with document upload as the higher tier — Rosie gates "Custom agent training — upload your training docs, policies" to its top plan ([heyrosie.com](https://heyrosie.com)); My AI Front Desk uses pasted FAQ-style pairs with "no structured training process" ([AINORA](https://ainora.lt/blog/my-ai-front-desk-review-alternatives-2026)). Realistic scoping matters: IONOS documents that its scraper "cannot read text trapped inside flat images, PDFs (like uploaded paper menus), or highly dynamic third-party scripts" ([IONOS Help](https://www.ionos.com/help/ai/general-information/setting-up-your-ai-receptionist)).

**5. Hours, holidays and timezone.** Set business hours with distinct after-hours behaviour; timezone is required for correct time-based answers ([ai-receptionist.com Help Center](https://ai-receptionist.com/help)). My AI Front Desk lets owners specify active hours "like during your business hours or on specific holidays" ([myaifrontdesk](https://www.myaifrontdesk.com/blogs/demystifying-my-ai-front-desk-pricing-a-2026-guide)). Multi-location products give **each location its own hours, routing rules, greetings and SMS responses** ([RingCentral](https://www.ringcentral.com/ai-receptionist.html)).

**6. Transfer and routing rules.** The documented control set is richer than most builders expect. One help centre lists: "configure whether you want call transfers enabled, set transfer rules, **enable warm transfer SMS notifications**, adjust **timeout settings**, and choose between **phone network or SIP/VoIP**" ([ai-receptionist.com](https://ai-receptionist.com/help)). Escalation depth is a tier lever — Rosie's Growth plan adds "Waterfall transfers — Rosie tries multiple numbers until someone answers" ([heyrosie.com/pricing](https://heyrosie.com/pricing)). For verticals, routing by topic/urgency is the expectation: "Route personal lines calls to one agent and commercial inquiries to another… Active claim emergency? Transfer immediately" ([Dialnote](https://dialnote.com/en/blog/ai-receptionist-insurance-growth)).

**7. Notification settings.** Per-event SMS/email/push to one or more recipients. Dialzara includes "Email/SMS notifications after each handled call" on every plan ([dialzara.com/pricing](https://dialzara.com/pricing)); Rosie ships "Email & text notifications for every call" on all plans ([heyrosie.com](https://heyrosie.com)). Configurable "instant alerts or daily summaries", plus real-time or scheduled SMS, is the documented norm ([ai-receptionist.com](https://ai-receptionist.com/help)). **Speed is the whole point** — as one buyer's guide puts it, "A daily summary email at 5 PM doesn't help when a business owner called at 10 AM" ([Dialnote](https://dialnote.com/en/blog/ai-receptionist-insurance-growth)).

**8. Number provisioning and forwarding.** Two models coexist: **provision a number in-app** (Dialzara includes "US-based phone number (local or toll-free)" on every plan, with UK/AU/NZ on request — [dialzara.com/pricing](https://dialzara.com/pricing); Rosie lets you "Pick your area code" — [heyrosie.com](https://heyrosie.com)) versus **forward from the existing number**, which is what most SMBs actually do. Note Rosie "requires you to forward calls from an existing phone system rather than provisioning a number" ([Retell's review](https://www.retellai.com/blog/best-ai-virtual-receptionists-appointment-scheduling)). The forwarding options the dashboard must expose: forward *all* calls, *after-hours/weekend only*, or *only when busy or unanswered* ([Nextiva](https://www.nextiva.com/blog/how-to-set-up-ai-receptionist.html)).

**9. Usage, billing and analytics.** See §2.2.

**Beyond the nine — differentiators seen only on leaders:**
- **Native mobile app.** Rosie ships iOS and Android **free on all plans**, with push alerts, unified inbox, recordings with AI summaries, **full agent configuration from the phone** ("Train and customize your AI receptionist directly from your phone — no computer required. Configure appointment scheduling, FAQ management, call transfers"), a guided mobile Quick Start, and **team management with role-based permissions** ([Rosie on the App Store](https://apps.apple.com/us/app/rosie-ai-business-receptionist/id6757593086); [Rosie mobile page](https://heyrosie.com/features/mobile-app)). Goodcall is web-dashboard only.
- **Team/user roles.** Goodcall meters team members as a plan feature (9 / 50 by tier) alongside "directory contacts" (25 / 500) ([goodcall.com/pricing](https://www.goodcall.com/pricing)).
- **Voice self-administration.** One product ships "Boss Mode — call in and manage your AI verbally with PIN or verbal password security"; the owner calls from their configured number, says "Boss Mode", and manages the account by voice ([ai-receptionist.com](https://ai-receptionist.com/help)). A genuinely novel pattern for a phone-first product.
- **Caller memory** across calls (Dialzara, Avoca, Allo) — see §1.4.

### 2.2 Analytics — what "standard" actually contains

The baseline is call volume plus duration. The competitive standard adds outcomes and operational signal:

| Metric | Evidence |
|---|---|
| Call volume, daily and monthly, with trend charts | "today's statistics with 7-day trend plots… plus current month totals with 6-month trend charts" ([ai-receptionist.com](https://ai-receptionist.com/help)) |
| Average call duration, total minutes used | same |
| **Unresolved calls** | same — "Track call volume, average call duration, total minutes used, and unresolved calls to optimize your AI's efficiency" |
| **Peak calling hours** | same — "Monitor peak calling hours and identify busy periods to plan accordingly" |
| **Resolution rate + which action produced resolution** | RingCentral's dashboard shows "call volume, resolution rates, and which actions led to resolution, such as referencing the knowledge base" ([RingCentral support](https://support.ringcentral.com/article-v2/intro-to-ai-receptionist-analytics.html)) |
| **Transfer rate and transfer success rate** | Webex's report shows "whether callers are reaching AI Receptionist, whether calls are being answered, how often calls are transferred, and whether transfers succeed" — and interprets them: "a low intent-transfer rate may indicate that intents need clearer descriptions, while a low transfer success rate may indicate that transfer destinations, operating hours, or routing configuration need review" ([Webex Help](https://help.webex.com/en-us/article/4chov0/AI-Receptionist-in-Webex-Calling)) |
| **Outcomes / work created** | Jobber's Receptionist dashboard has an **Outcomes card** visualising "the work created by your Receptionist, including jobs, requests, and tasks", plus a **Time saved** card showing minutes and calls/texts answered ([Jobber Help Center](https://help.getjobber.com/en/articles/receptionistpowered-by-jobber-ai)) |
| **On-site answer rate** (how well *humans* answer) | Slang.ai "gives you insight into how your front of house performs by measuring your on-site answer rate" ([slang.ai](https://www.slang.ai/blog-slang-ai-vs-hostie-complete-2026-comparison-guide)) |
| CSAT captured natively on every call | Slang.ai Core ([slang.ai/pricing](https://www.slang.ai/pricing)) |
| Comparison across locations | Webex — "compare performance across receptionists or locations" |
| Voice vs message split | Weave toggles Voice/Message tabs ([Weave Help](https://www.weavehelp.com/weavehelp/article/ai-receptionist-analytics)) |

Two observations worth acting on. First, **advanced analytics is commonly a paid tier** — "Advanced analytics features with detailed trends are available on [higher plans]" ([ai-receptionist.com](https://ai-receptionist.com/help)). Second, **the best analytics are prescriptive, not descriptive**: Webex tells the admin what a bad number *means* and what to change. That is a cheap, high-value differentiator — a metric with a recommended fix attached beats a chart.

**Retention is a dashboard feature, not just a compliance setting.** Goodcall meters "call & customer details" at 7 days / 30 days / unlimited by tier ([goodcall.com/pricing](https://www.goodcall.com/pricing)), and a tester flagged the 7-day floor as unusable for settling a two-week-old dispute ([Lunacal](https://lunacal.ai/blogs/ai-receptionist)). See §3.7.

**Usage and billing.** The dashboard must show minutes/calls used against the plan and the overage position, because every pricing model in §5 punishes surprises. My AI Front Desk lets owners cap spend — "setting limits on how many minutes the AI receptionist can be active, making your monthly bills more predictable" ([myaifrontdesk](https://www.myaifrontdesk.com/blogs/demystifying-my-ai-front-desk-pricing-a-2026-guide)). **A spend cap is a genuine trust feature**, and its absence is a documented complaint against Rosie, whose auto-upgrade has "no spend cap" ([CloudTalk](https://www.cloudtalk.io/blog/rosie-ai-answering-service-pricing)). Note also that **test calls may consume billable minutes** — RingCentral warns "test calls count towards your monthly minutes" ([RingCentral support](https://support.ringcentral.com/article-v2/setting-up-your-ai-receptionist-in-the-admin-portal.html)) — while Smith.ai instead meters test and simulated calls as separate plan entitlements ([smith.ai](https://smith.ai/pricing/ai-receptionist)).

### 2.3 Onboarding — the under-15-minute standard

**This is the most consequential single benchmark in the report.** The category has standardised on a website-scrape-driven, self-serve wizard, and buyers now expect to be live the same hour.

**The canonical flow, from vendors' own pages:**

**Rosie — three steps** ([heyrosie.com](https://heyrosie.com)):
> **Step 1: "Add your site or Google Business Profile to train Rosie."** — "Rosie automatically learns your business' hours, services, and basic information in seconds."
> **Step 2: "Review and customize your information."**
> **Step 3: "Start sending your calls to Rosie."** — "Once you're happy with the setup, forward your calls to Rosie and she'll start right away."

Rosie's four-step variant adds explicit confirmation ("Confirm Rosie has things right") and notes you can seed from "your Google Business profile, website address, or basic business details, such as your company name, industry, and services" ([Rosie AI receptionist page](https://heyrosie.com/solutions/ai-receptionist)). Rosie's own claim: "your Rosie AI agent could be ready to answer her first customer call in just a few minutes. Just confirm your business details, describe your services, share some answers to FAQs, and set your custom greeting" ([Rosie blog](https://heyrosie.com/blog/best-ai-answering-service)). Independent confirmation from a user: *"you just provide them your website URL and your Google My Business Profile and they learn everything about your business"* (8-month user, [Facebook trade group post, 2025-08-01](https://www.facebook.com/groups/windowtint/posts/24378249451799182)).

**Trillet — five steps** ([Trillet](https://trillet.ai/blogs/ai-receptionist-setup-without-technical-knowledge)):
> 1. Paste your website URL — "The AI reads your site to understand your services, hours, and FAQs"
> 2. **Reviews are gathered automatically** — "the platform pulls your business reviews to understand what customers say about you"
> 3. Choose your phone number (new, or forward existing)
> 4. Review and customise
> 5. Go live

**IONOS — a documented 5-step wizard with a hard gate** ([IONOS Help](https://www.ionos.com/help/ai/general-information/setting-up-your-ai-receptionist)):
> Business info → **automated scraping** ("Enter your primary business website URL and click Train. The system will take approximately 2 to 3 minutes to crawl and map your site", with an explicit "I don't have a website" bypass) → **greeting script customisation** ("The AI dynamically pre-populates an initial greeting script based on the identity and data captured") → … → **Step 5: mandatory voice activation test.** *"Before your AI Receptionist can go live, you must complete a voice verification test. This is a mandatory step; deployment remains locked until a test call is successfully placed."* The documented test scenarios: business hours/location, an appointment or product inquiry, an SMS summary request, and **triggering a live transfer to verify the escalation line**.

**Published time-to-live claims:**

| Vendor | Claim | Source |
|---|---|---|
| Dialzara | **~2 minutes** from a website URL | [dialzara.com](https://dialzara.com/compare/dialzara-vs-ringcentral) (vendor) |
| Dialzara | **~8 minutes**, measured — "describe your business, pick a voice, and forward your number" | [Cekura](https://www.cekura.ai/blogs/ai-answering-service) |
| Dialzara | **~15 minutes**, measured — "drop in the business URL, pick a voice from the 50+ library, forward your phone number, you're live" | [Retell's review](https://www.retellai.com/blog/best-ai-virtual-receptionists-appointment-scheduling) |
| My AI Front Desk | **15 minutes** — fastest of seven tools tested | [BestAgentPick](https://bestagentpick.com/best-ai-agents-appointment-booking-2026) |
| Rosie | **~20 minutes**, measured | [Retell's review](https://www.retellai.com/blog/best-ai-virtual-receptionists-appointment-scheduling) |
| Synthflow | **under 30 minutes**, measured from clean signup | [tested.media](https://tested.media/synthflow-ai-review) |
| Goodcall | "self-service setup in minutes" | [Sonant](https://www.sonant.ai/blog/goodcall-alternative-review) |
| Retell AI | signup to live agent in **under a single workday** | [Retell](https://www.retellai.com/blog/best-call-management-solutions-2026) (vendor) |
| Loman | live in **under 24 hours** | [Loman](https://loman.ai/blog/loman-ai-review-pricing-features) |
| Synthflow Enterprise | **3 weeks** | [tooliverse](https://tooliverse.ai/tools/synthflow) |

**The honest counter-view, which we should take seriously.** A more realistic breakdown puts *minimum viable* setup at **75 minutes** (account, phone config, greeting, basic routing) with knowledge-base upload at 30–45 minutes and a three-day arc to full deployment including integrations and testing ([NextPhone](https://www.getnextphone.com/blog/ai-receptionist-setup-time)). And Trillet — a vendor selling the 5-minute scrape — concedes the real work is elsewhere: *"The real work is not technical, it is the 15 minutes of reviewing what the AI scraped, correcting your hours and services, connecting a calendar, and choosing which calls to forward. **Skip that review pass and you get a fast setup with day-one mistakes**"* ([Trillet](https://trillet.ai/blogs/ai-receptionist-setup-without-technical-knowledge)).

**Test-calling before go-live is a documented standard, not a nicety.** IONOS makes it a hard gate; RingCentral makes it the final setup step ([RingCentral](https://support.ringcentral.com/article-v2/setting-up-your-ai-receptionist-in-the-admin-portal.html)); Retell ships Simulation Testing on the free tier ([retellai.com/pricing](https://www.retellai.com/pricing)); Smith.ai sells test and simulated calls as plan entitlements ([smith.ai](https://smith.ai/pricing/ai-receptionist)). Nextiva prescribes three to five test calls covering a service from the website, hours/policies, a specific FAQ, **and "something the AI shouldn't know or shouldn't be discussing (to test guardrails)"** — noting "This takes five minutes and prevents embarrassing mistakes with real callers" ([Nextiva](https://www.nextiva.com/blog/how-to-set-up-ai-receptionist.html)).

**Templates by vertical** are the other accelerator: Synthflow ships templates for HVAC after-hours, dental booking, law-firm intake, real-estate qualification, restaurant reservation, e-commerce order status, salon and fitness scheduling, "and 12 more" ([tested.media](https://tested.media/synthflow-ai-review)); Dialzara ships **88+ pre-built industry prompts** ([AINORA](https://ainora.lt/blog/dialzara-review-alternatives-2026)); Retell ships pre-built agent templates on the free tier ([retellai.com/pricing](https://www.retellai.com/pricing)).

**One onboarding step that cannot be fast, and nobody advertises:** A2P 10DLC registration for SMS. Twilio's own quickstart currently warns *"campaign reviews take 10–15 days"*, and from 30 June 2026 registration additionally requires a public privacy policy URL and terms URL ([Twilio Docs](https://www.twilio.com/docs/messaging/compliance/a2p-10dlc/quickstart); [Twilio Changelog](https://www.twilio.com/en-us/changelog/a2p-10dlc-campaign-registration-will-require-privacy-policy-and-)). See §3.4. **Voice can be live in 15 minutes; SMS follow-up cannot.** Any onboarding flow we ship must sequence around this rather than pretend it away.

### 2.4 Table stakes vs differentiators

**Table stakes — nearly every vendor has these, and their absence disqualifies:**
1. Call log with recording playback, transcript and AI summary on every call
2. Structured message/lead capture, viewable and exportable
3. Greeting, agent name and voice selection
4. Editable business info / FAQ knowledge base
5. Business hours with after-hours behaviour, and timezone
6. At least one transfer destination with rules
7. Email + SMS notification per call, to at least one recipient
8. Call-forwarding instructions or in-app number provisioning
9. Usage against plan, visible before the invoice
10. Basic call-volume analytics
11. Self-serve setup from a website URL in well under an hour
12. A test-call path before going live

**Differentiators — only leaders have them:**
1. **Native mobile app with full configuration** (Rosie)
2. **Prescriptive analytics** that name the fix, not just the number (Webex)
3. **Outcome analytics** — jobs/requests created, not just calls handled (Jobber, Slang CSAT)
4. **On-site answer rate** — measuring the human team, not just the AI (Slang.ai)
5. **Waterfall/multi-number escalation** (Rosie Growth)
6. **Warm-transfer briefing** to the human before connecting (Rosie, Dialzara, Sameday, Synthflow)
7. **Caller memory across calls** (Dialzara, Avoca, Allo)
8. **AI-assisted agent configuration** (Dialzara's AI Configurator)
9. **Voice self-administration** (Boss Mode)
10. **Per-location config from one dashboard** (Goodcall, RingCentral, Slang.ai)
11. **Team roles and permissions** (Goodcall, Rosie mobile)
12. **Spend caps** (My AI Front Desk)
13. **Simulated-call regression testing as a shipped feature** (Retell, Smith.ai)

> **Sourcing note for this section.** Vendor help centres were prioritised (Goodcall, Rosie, IONOS, RingCentral, Webex, Jobber, Weave, ai-receptionist.com, Twilio) over review blogs. Where only a review blog documents a capability, it is cited as such. No screenshots were viewed; UI descriptions come from documented text, and none are reconstructed from memory. Capabilities not found in any documentation are simply absent from the lists above rather than assumed either way.
...
20. **[MUST]** Hours/holidays editor. *(§2.1)*
...
27. **[SHOULD]** **Spend cap** the owner controls. Absence is a documented complaint against a market leader. *(§2.2)*
...
31. **[NICE]** Native mobile app with push alerts and full agent configuration from the phone. *(§2.1)*
...
34. **[MUST]** A **test-call path before go-live** — ideally a hard gate, as IONOS does. Prescribe the scenarios: an FAQ, hours/policy, a booking, a transfer to verify the escalation line, **and an out-of-scope question to test guardrails**. *(§2.3)*
